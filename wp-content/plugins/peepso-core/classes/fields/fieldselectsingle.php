...
class PeepSoFieldSelectSingle extends PeepSoField {
	...
	protected function _render_form_select( )
	{
		...
		ob_start();
		?>
		<select<?php echo $this->_render_input_args() . $this->_render_required_args(); ?>>
	...
	protected function _render_form_checklist()
	{
		...
		foreach ($options as $k => $v) {
			...
			$option = '<div class="ps-checkbox"><input name="%4$s" type="radio" id="%4$s-%1$s" %3$s value="%1$s" ' . $this->_render_input_args() . $this->_render_required_args() . ' /> <label for="%4$s-%1$s">%2$s</label></div>';
	...
	protected function _render_form_checklist_register()
	{
		...
		foreach ($options as $k => $v) {
			...
			$option = '<div class="ps-checkbox"><input name="%4$s" type="radio" %3$s value="%1$s" id="%1$s" ' . $this->_render_input_register_args() . $this->_render_required_args() . ' /> <label for="%1$s">%2$s</label></div>';
