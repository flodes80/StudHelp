				...
				foreach ($peepso_vars as $key=>$value) {
					...
					elseif ('meta_' == substr($key,0,5)) {
						...
						if ($key == "peepso_user_field_gender") {
							$wp_user_query->query_where .= $wpdb->prepare('AND ID IN (SELECT user_id FROM ' . $wpdb->usermeta . ' WHERE `meta_key` = "%s" AND CAST(`meta_value` AS CHAR) = %s AND user_id IN (SELECT user_id FROM ' . $wpdb->usermeta . ' WHERE `meta_key` = "%s" AND `meta_value` <> %d AND IF (`meta_value` = %d, %d > 0, TRUE))) ', $key, $value, $key . '_acc', PeepSo::ACCESS_PRIVATE, PeepSo::ACCESS_MEMBERS, $user_id);
						} else {
							$wp_user_query->query_where .= $wpdb->prepare('AND ID IN (SELECT user_id FROM ' . $wpdb->usermeta . ' WHERE `meta_key` = "%s" AND CAST(`meta_value` AS CHAR) = %s) ', $key, $value);
						}
					} else {
						if ($key == 'cover_custom') {
