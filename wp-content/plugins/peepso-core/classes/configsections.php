...
class PeepSoConfigSections extends PeepSoConfigSectionAbstract
{
    ...
    private function integration() {
        $this->set_field(
            'social_login_header',
            __('TwistPress Social Login integration', 'peepso-core'),
            'separator'
        );

        if ( !PeepSo::social_login_enabled() ) {
            $url = '<a href="https://peep.so/sociallogin" target="_blank">here</a>';
            $this->set_field(
                'social_login_integration_enable_description',
                sprintf(__('Social Login is a plugin created by TwistPress.com. For the integration to work, please make sure you have the plugin installed and activated. You can get it %s.', 'peepso-core'), $url),
                'message'
            );
        } else {
            $this->set_field(
                'social_login_integration_enable',
                __('Enable TwistPress Social Login integration (BETA)', 'peepso-core'),
                'yesno_switch'
            );
        }
