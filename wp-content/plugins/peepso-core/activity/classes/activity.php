...
//        }
    ...
    public function post_edit_notice() {
        ...
        if(strlen($edit_date = get_post_meta($post->ID, 'peepso_last_edit', TRUE))){ ?>
            ...
            <?php
            $edit_timestamp_user_offset = $edit_date + 3600 * PeepSoUser::get_gmt_offset(get_current_user_id());
            $edit_date_user_offset = date( get_option('date_format'). ' ' . get_option('time_format'), $edit_timestamp_user_offset);
            ?>
    ...
    public function post_actions()
    {
        ...
        if (PeepSo::get_option('activity_social_sharing_enable', 0)) {
            $acts['share'] = array(
                ...
                'class' => 'actaction-share',
                'icon' => 'share-alt',
                'click' => "share.share_url('" . PeepSo::get_page('activity_status') . $post->post_title . "');",
                'allow_guest' => TRUE
            );
        }
...
//        }
     ...
     * @param int $act_id
     */
    public function handle_embed_data($post_id, $act_id)
    {
        ...
        $embed_url = $this->_input->value('embed', NULL, FALSE);
        if (!$embed_url) {
            return;
        }
